...
class CycleService {
    ...
    static async getById(id, adminId) {
        ...
        if (adminId && cycle.admin_id !== adminId)
            throw new middlewares_1.ApiError(403, 'cycle.notYours');
        return cycle;
    }
    /**
     * Create a new revenue cycle — tied to admin
     */
    static async create(month, exchangeRate, adminId) {
        // Check duplicate for this admin
        const existing = await db.revenueCycle.findFirst({
            where: { month, admin_id: adminId || null },
        });
        if (existing)
            throw new middlewares_1.ApiError(409, 'cycle.alreadyExists');
        ...
        const activeKocs = await database_1.default.kOC.findMany({
            where: { status: 'ACTIVE', ...(adminId ? { admin_id: adminId } : {}) },
            select: { id: true },
        });
        if (activeKocs.length > 0) {
            await database_1.default.revenueRecord.createMany({
     ...
     */
    static async addKocs(cycleId, kocIds, adminId) {
        ...
        const targetKocs = await database_1.default.kOC.findMany({
            where: {
                status: 'ACTIVE',
        ...
        if (targetKocs.length === 0)
            return { added: 0, skipped: 0 };
        // Find which ones are already in the cycle
        const existing = await database_1.default.revenueRecord.findMany({
     ...
     */
    static async reopen(id, adminId) {
        const cycle = await db.revenueCycle.findUnique({ where: { id } });
        if (!cycle)
            throw new middlewares_1.ApiError(404, 'cycle.notFound');
        if (adminId && cycle.admin_id !== adminId)
            throw new middlewares_1.ApiError(403, 'cycle.notYours');
        if (cycle.status === 'OPEN')
            throw new middlewares_1.ApiError(400, 'cycle.alreadyOpen');
     ...
     */
    static async complete(id, adminId) {
        const cycle = await db.revenueCycle.findUnique({ where: { id } });
        if (!cycle)
            throw new middlewares_1.ApiError(404, 'cycle.notFound');
        if (adminId && cycle.admin_id !== adminId)
        ...
        if (cycle.status !== 'LOCKED')
            throw new middlewares_1.ApiError(400, 'cycle.mustLockFirst');
        // Determine which KOCs have hit the threshold (accumulated >= min_payment)
        const paymentStatus = await revenue_service_1.RevenueService.getPaymentStatus(id, adminId);
    ...
    static async delete(id, adminId) {
        ...
        if (adminId && cycle.admin_id !== adminId)
            throw new middlewares_1.ApiError(403, 'cycle.notYours');
        if (cycle.status !== 'OPEN')
            throw new middlewares_1.ApiError(400, 'cycle.onlyOpenCanDelete');
        const recordCount = await database_1.default.revenueRecord.count({ where: { cycle_id: id } });
        if (recordCount > 0)
            throw new middlewares_1.ApiError(400, 'cycle.hasRecords');
