    ...
    try {
      ...
      if (config.autoScrapeRevenue) {
        try {
          ...
          // GemLogin / CDP mode: CHROME_CDP_URL đã được inject bởi GemLoginService.startProfile()
          // → bỏ qua check youTubeSession, dùng trực tiếp adminId
          let effectiveAdminId: string;
          if (process.env.CHROME_CDP_URL) {
            effectiveAdminId = adminId || '';
            logger.info(`[CronJob] GemLogin/CDP mode active — skipping session check, using admin ${effectiveAdminId || '(default)'}`);
          } else {
            ...
            effectiveAdminId = adminSession.admin_id;
