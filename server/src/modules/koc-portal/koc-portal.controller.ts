...
export class KocPortalController {
  /**
   * GET /api/koc-portal/my-revenue
   * Returns all revenue records for the logged-in KOC user, grouped by cycle
   */
  static async getMyRevenue(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      const t = (req as any).t;
      const kocId = req.user?.kocId;

      if (!kocId) {
        throw new ApiError(403, 'auth.noKocLinked');
      }
  ...
  static async getMyStats(req: AuthenticatedRequest, res: Response, next: NextFunction): Promise<void> {
    try {
      ...
      const latestScrape = await prisma.youTubeScrapeResult.findFirst({
        where: { koc_id: kocId },
        orderBy: { scraped_at: 'desc' },
      });

      // Get monthly revenue analytics
      const monthlyAnalytics = await prisma.monthlyRevenueAnalytics.findMany({
        where: { koc_id: kocId },
