...
interface CompanyVsKocChartProps {
  trendData: any[];
  loading: boolean;
  chartColors: { grid: string; text: string; tooltipBg: string; tooltipBorder: string };
}

const CompanyVsKocChart: React.FC<CompanyVsKocChartProps> = ({ trendData, loading, chartColors }) => {
  ...
  return (
    <Card title={t('dashboard.companyShare') + ' ' + t('common.vs') + ' ' + t('dashboard.kocPayUSD')} loading={loading} style={{ borderRadius: 12, border: '1px solid #722ed1' }}>
      <ResponsiveContainer width="100%" height={300}>
        <BarChart data={trendData || []}>
          ...
          <Legend />
          <Bar dataKey="totalCompanyShare" name={t('dashboard.companyShare')} fill="#1677ff" radius={[8, 8, 0, 0]} />
...
export default CompanyVsKocChart;
