...
type ModuleMeta = Omit<
  ModuleNode,
  'radius' | 'angle' | 'speed' | 'size' | 'elev' | 'metric' | 'metricLabel'
>;
  ...
  return (
    <group>
      ...
      <Billboard position={[0, 0, 0]}>
        <Text fontSize={1.3} color="#ffffff" anchorX="center" anchorY="middle" outlineWidth={0.03} outlineColor="#0a0a15">{activity}</Text>
      </Billboard>
      <Billboard position={[0, -0.95, 0]}>
        <Text fontSize={0.22} color="#c0a080" anchorX="center" anchorY="top" outlineWidth={0.015} outlineColor="#000">{perMinuteLabel}</Text>
      </Billboard>
      <Billboard position={[0, -2.6, 0]}>
        <Text fontSize={0.24} color="#e0c4a0" anchorX="center">{title}</Text>
      </Billboard>
    </group>
  );
}
    ...
    <group ref={groupRef}>
      ...
      {showLabel && (
        <Billboard position={[0, m.size + 0.45, 0]}>
          ...
          <Text fontSize={0.15} color={m.color} anchorX="center" anchorY="top" position={[0, -0.05, 0]}>
...
function CentralConnectionBeam({ module, index }: { module: ModuleNode; index: number }) {
  ...
  useFrame(({ clock }) => {
    ...
    const endV = new THREE.Vector3(endX, endY, endZ);
    const startV = endV.clone().normalize().multiplyScalar(1.6);
    const axisLen = endV.distanceTo(startV);

    // Tangent in horizontal plane → sweeps sideways like an orbital trajectory
    const tangent = new THREE.Vector3(-endZ, 0, endX).normalize();
...
function ModuleConnectionBeam({ from, to, color }: { from: ModuleNode; to: ModuleNode; color: string }) {
  ...
  const getPos = (m: ModuleNode, clock: number): THREE.Vector3 => {
    ...
    return new THREE.Vector3(Math.cos(t) * m.radius, m.elev, Math.sin(t) * m.radius);
  };
